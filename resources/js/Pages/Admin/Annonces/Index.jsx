                        ...
                        <div className="flex flex-wrap gap-2">
                            {abilities.map((ability) => (
                                <PermissionCheckbox
                                    ...
                                    label={abilityLabel(ability).replace('Annonces - ', '')}
                                />
                            ))}
                        </div>
                    </div>
                            ...
                            {abilities.map((ability) => (
                                <label key={ability} className="block">
                                    <span className="mb-1 block text-[11px] font-black uppercase tracking-[0.08em] text-[var(--app-muted)]">
                                        {abilityLabel(ability).replace('Annonces - ', '')}
                                    </span>
                                    <select
...
export default function AdminAnnoncesIndex({
    abilities = [],
    roles = [],
    sectors = [],
    users = [],
    ...
    const submit = () => {
        form.put(route('admin.annonces.update'), {
            preserveScroll: true,
        });
    };
    ...
    const header = (
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <h1 className="text-[22px] leading-none font-black uppercase tracking-[0.06em]">Droits Annonces</h1>
    ...
    return (
        <AppLayout title="Droits Annonces" header={header}>
            <Head title="Droits Annonces" />

            <div className="space-y-5">
                <div className="rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface)] p-4 text-sm text-[var(--app-muted)]">
                    <strong className="text-[var(--app-text)]">Voir</strong> donne accès à la page Annonces.
                    {' '}<strong className="text-[var(--app-text)]">Créer et envoyer</strong> autorise la création, l'envoi et les brouillons.
                    {' '}<strong className="text-[var(--app-text)]">Gérer toutes les annonces</strong> autorise à voir/modifier/supprimer les annonces de tous les utilisateurs.
                </div>
