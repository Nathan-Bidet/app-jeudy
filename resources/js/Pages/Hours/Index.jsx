...
import {
    ...
    OvertimeBadge,
    formatHourSheetDate as formatHistoryDate,
...
export default function HoursIndex({
    ...
    useEffect(() => {
        if (highlightId) {
            setDetailSheetId(Number(highlightId));
        }
    }, [highlightId]);
    ...
    const detailEntry = useMemo(() => {
        ...
        const sheet = hourSheets.find((candidate) => Number(candidate?.id) === Number(detailSheetId)) || null;

        return sheet ? { sheet, leave: approvedLeavesByDate[sheet.work_date] || null } : null;
    }, [detailSheetId, hourSheets, approvedLeavesByDate]);
    ...
    return (
        ...
        >
            ...
            <HourSheetDetailModal
                sheet={detailEntry?.sheet ?? null}
                leave={detailEntry?.leave ?? null}
